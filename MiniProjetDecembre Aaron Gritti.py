...
def Easter6():
    ...
    Mhhhbg="orange"
    if PartieRapide==True:             # Si on a relancé une partie à partir de BoutonEaster
        Mhhhbg="white"
    Mhhh.config(bg=Mhhhbg,relief="groove",command=EasterWin)
    i=1
    print(i)
    EasterEggGagné=False                # Si on avait mis -if EasterEggGagné==False- ici, Mhhh aurait continué de se déplacer et on l'aurait vu si on avait réouvert JeuRègles avant la fin des 15 mouvements de Mhhh
    for i in range(1,16):
        if EasterEggGagné==False:       # La boucle ne continuera pas si l'easter egg est gagné
            i+1
            X=randrange(20,500)
            Y=randrange(40,500)
            Mhhh.place(x=X,y=Y)
            time.sleep(0.85)            # Rapidité de changement de position de Mhhh
        if EasterEggGagné==False:       # Evite un dernier print après avoir appuyé sur le bouton (car appuyé pendant le time.sleep)
            print(i+1)                  # Voir le nombre de mouvements de Mhhh depuis le début du jeu (dans la console)
    if EasterEggGagné==False:           # Si on a pas gagné, on perd (c'est logique...non ?)
...
def PasDeJeuSansEasterEgg():
    ...
    EasterEggTrouvé=True
    GO.destroy()
    Mhhh.config(command=Easter2)
    Egg1 = threading.Timer(1.0, Easter1)        # Execute Easter1 après 1 seconde
    Egg1.start()
    Egg2 = threading.Timer(3.0, Easter3)        # Execute Easter3 après 3 secondes ( 2 secondes après l'execution de Easter1 )
    Egg2.start()
...
Terminé=False
QuitterAnnulé=False
EasterEggTrouvé=False
EasterEggGagné=False
PartieRapide=False
# Points de passage
...
ImgVersus = PhotoImage(file=r'c:\Users\aaron\Documents\All the past Python shit\NSI\Save Python\versus.png')
ImgPierre = PhotoImage(file=r'c:\Users\aaron\Documents\All the past Python shit\NSI\Save Python\pierre.png')
ImgCiseaux = PhotoImage(file=r'c:\Users\aaron\Documents\All the past Python shit\NSI\Save Python\ciseaux.png')
ImgFeuille = PhotoImage(file=r'c:\Users\aaron\Documents\All the past Python shit\NSI\Save Python\feuille.png')
